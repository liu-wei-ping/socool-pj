/*
 * ! jScrollPane - v2.0.14 - 2013-05-01 http://jscrollpane.kelvinluck.com/
 * 
 * Copyright (c) 2010 Kelvin Luck Dual licensed under the MIT or GPL licenses.
 */
(function(b, a, c) {
	b.fn.jScrollPane = function(e) {
		function d(D, O) {
			var ay, Q = this, Y, aj, v, al, T, Z, y, q, az, aE, au, i, I, h, j, aa, U, ap, X, t, A, aq, af, am, G, l, at, ax, x, av, aH, f, L, ai = true, P = true, aG = false, k = false, ao = D
					.clone(false, false).empty(), ac = b.fn.mwheelIntent
					? "mwheelIntent.jsp"
					: "mousewheel.jsp";
			...
			function ar(aQ) {
				...
				if (Y === c) {
					...
					v = D.innerHeight();
					D.width(aj);
					Y = b('<div class="jspPane" />').css("padding", aH)
							.append(D.children());
					...
					al
							.append(b('<div class="jspVerticalBar" />')
									.append(
											...
											b('<div class="jspTrack gradient" />')
													.append(b('<div class="jspDrag gradient" />')
															.append(
			...
			function ah() {
				al
						.find(">.jspHorizontalBar>.jspCap:visible,>.jspHorizontalBar>.jspArrow")
						.each(function() {
									l -= b(this).outerWidth()
								});
				G.width(l + "px");
			...
			function F() {
				if (aE && az) {
					...
					v -= s;
					aj -= aI;
					G.parent().append(b('<div class="jspCorner" />').css(
							"width", aI + "px"));
					o();
					ah()
				}
					...
					ap.bind("mousedown.jsp", function(aN) {
						if (aN.originalTarget === c
							...
							var aL = b(this), aO = aL.offset(), aM = aN.pageY
								...
								var aR = aL.offset(), aS = aN.pageY - aR.top
										- A / 2, aP = v * ay.scrollPagePercent, aQ = i
										* aP / (Z - v);
								if (aM < 0) {
									if (I - aQ > aS) {
										Q.scrollByY(-aP)
					...
					G.bind("mousedown.jsp", function(aN) {
						if (aN.originalTarget === c
							...
							var aL = b(this), aO = aL.offset(), aM = aN.pageX
								...
								var aR = aL.offset(), aS = aN.pageX - aR.left
										- at / 2, aP = aj
										* ay.scrollPagePercent, aQ = j * aP
										/ (T - aj);
								if (aM < 0) {
									if (aa - aQ > aS) {
										Q.scrollByX(-aP)
			...
			function S() {
				...
				D.attr("tabindex", 0).unbind("keydown.jsp keypress.jsp").bind(
						"keydown.jsp", function(aN) {
							if (aN.target !== this
									&& !(aK.length && b(aN.target).closest(aK).length)) {
								return
							}
							var aM = aa, aL = I;
							switch (aN.keyCode) {
			...
			function R() {
				D.attr("tabindex", "-1").removeAttr("tabindex")
			...
			function m() {
				...
				b(document.body).delegate("a[href*=#]", "click", function(s) {
					...
					if (location.href.indexOf("#") !== -1) {
						aK = location.href
								.substr(0, location.href.indexOf("#"))
					}
					if (aI !== aK) {
						return
					}
			...
			function an() {
				...
				al
						.unbind("touchstart.jsp touchmove.jsp touchend.jsp click.jsp-touchclick")
		...
		b.each(["arrowButtonSpeed", "trackClickSpeed", "keyboardSpeed"],
				function() {
