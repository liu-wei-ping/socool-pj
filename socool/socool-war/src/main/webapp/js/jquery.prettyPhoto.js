...
(function($) {
	...
	$.fn.prettyPhoto = function(pp_settings) {
		pp_settings = jQuery.extend({
			hook : 'rel', /*
							 * the attribute tag to use for prettyPhoto hooks.
							 * default: 'rel'. For HTML5, use "data-rel" or
							 * similar.
							 */
			...
			allow_resize : true, /*
									 ...
									 */
			allow_expand : true, /*
									 * Allow the user to expand a resized image.
									 * true/false
									 */
			default_width : 500,
			default_height : 344,
			...
			theme : 'pp_default', /*
									 ...
									 * light_square / dark_square / facebook
									 */
			horizontal_padding : 20, /*
										 * The padding on each side of the
										 * picture
										 */
			hideflash : false, /*
								 * Hides all the flash object on a page, set to
								 * TRUE if flash appears over prettyPhoto
								 */
			wmode : 'opaque', /* Set the flash wmode attribute */
			autoplay : true, /* Automatically start videos: True/False */
			modal : false, /*
							 * If set to true, only the close button will close
							 * the window
							 */
			deeplinking : true, /*
			...
			overlay_gallery_max : 30, /*
										 * Maximum number of pictures in the
										 * overlay gallery
										 */
		...
		$.prettyPhoto.initialize = function() {
			...
			set_position = jQuery.inArray($(this).attr('href'), pp_images); // Define
																			// where
																			// in
																			// the
																			// array
																			...
																			// is
																			// positionned
			rel_index = (isSet) ? set_position : $("a[" + settings.hook + "^='"
					+ theRel + "']").index($(this));
		 ...
		 * 
		 * @param image
		 *            {String,Array} Full path to the image to be open, can also
		 *            be an array containing full images paths.
		 * @param title
		 *            {String,Array} The title to be displayed with the picture,
		 *            can also be an array containing all the titles.
		 * @param description
		 *            {String,Array} The description to be displayed with the
		 *            picture, can also be an array containing all the
		 *            descriptions.
		 */
		$.prettyPhoto.open = function(event) {
			...
			$pp_pic_holder.find('.currentTextHolder').text((set_position + 1)
					+ settings.counter_separator_label + $(pp_images).size());
			...
			if (movie_height.indexOf('%') != -1) {
				movie_height = parseFloat(($(window).height()
						* parseFloat(movie_height) / 100)
						- 150);
				percentBased = true;
			}
			if (movie_width.indexOf('%') != -1) {
				movie_width = parseFloat(($(window).width()
						* parseFloat(movie_width) / 100)
						- 150);
				percentBased = true;
			}
				...
				switch (_getFileType(pp_images[set_position])) {
					case 'image' :
						...
						imgPreloader.onload = function() {
							...
							pp_dimensions = _fitToViewport(imgPreloader.width,
									imgPreloader.height);
					...
					case 'youtube' :
						...
						toInject = settings.iframe_markup.replace(/{width}/g,
								pp_dimensions['width']).replace(/{height}/g,
								pp_dimensions['height']).replace(/{wmode}/g,
					...
					case 'vimeo' :
						...
						movie = 'http://player.vimeo.com/video/' + match[3]
						...
						toInject = settings.iframe_markup.replace(/{width}/g,
								vimeo_width).replace(/{height}/g,
								pp_dimensions['height']).replace(/{path}/g,
								movie);
						break;
					...
					case 'quicktime' :
						...
						pp_dimensions['containerHeight'] += 15; // Add space for
																// the control
																// bar

						toInject = settings.quicktime_markup.replace(
								/{width}/g, pp_dimensions['width']).replace(
								/{height}/g, pp_dimensions['height']).replace(
								/{wmode}/g, settings.wmode).replace(/{path}/g,
								pp_images[set_position]).replace(/{autoplay}/g,
								settings.autoplay);
						break;
					...
					case 'ajax' :
						doresize = false; // Make sure the dimensions are not
											// resized.
						pp_dimensions = _fitToViewport(movie_width,
								movie_height);
					...
					case 'inline' :
						// to get the item height clone it, apply default width,
						// wrap it in the prettyPhoto containers , then delete
		...
		$.prettyPhoto.changeGalleryPage = function(direction) {
			...
			slide_speed = (direction == 'next' || direction == 'previous')
			...
			$pp_overlay.fadeOut(settings.animation_speed, function() {
						...
						if (settings.hideflash)
							$('object,embed,iframe[src*=youtube],iframe[src*=vimeo]')
									.css('visibility', 'visible'); // Show the
																	// flash

						$(this).remove(); // No more need for the prettyPhoto
											// markup
		...
		function _showContent() {
			...
			$pp_pic_holder.animate({
						'top' : projectedTop,
						'left' : ((windowWidth / 2)
								- (pp_dimensions['containerWidth'] / 2) < 0)
								? 0
								: (windowWidth / 2)
						...
						$pp_pic_holder.find('.pp_fade')
								.fadeIn(settings.animation_speed); // Fade the
																	// new
																	// content
						...
						if (settings.allow_expand) {
							if (pp_dimensions['resized']) { // Fade the resizing
															// link if the image
															// is resized
						...
						if (settings.autoplay_slideshow && !pp_slideshow
								&& !pp_open)
							$.prettyPhoto.startSlideshow();
		...
		function _checkPosition(setCount) {
			(setCount > 1) ? $('.pp_nav').show() : $('.pp_nav').hide(); // Hide
																		// the
																		// bottom
																		// nav
																		// if
		...
		function _getDimensions(width, height) {
			...
			$pp_details = $pp_details.clone().addClass(settings.theme)
					.width(width).appendTo($('body')).css({
								'position' : 'absolute',
								'top' : -10000
							});
		...
		function _insert_gallery() {
			if (isSet && settings.overlay_gallery
				...
				itemWidth = 52 + 5; // 52 beign the thumb width, 5 being the
									// right margin.
				navWidth = (settings.theme == "facebook" || settings.theme == "pp_default")
						? 50
						: 30; // Define the arrow width depending on the theme

				itemsPerPage = Math.floor((pp_dimensions['containerWidth']
						- 100 - navWidth)
						/ itemWidth);
				...
				$pp_gallery.css('margin-left',
						...
						.find('div:first').width(galleryWidth + 5).find('ul')
		...
		function _build_overlay(caller) {
			...
			$pp_pic_holder = $('.pp_pic_holder'), $ppt = $('.ppt'), $pp_overlay = $('div.pp_overlay'); // Set
																										// my
																										// global
																										// selectors
			...
			if (isSet && settings.overlay_gallery) {
				...
				$pp_gallery = $('.pp_pic_holder .pp_gallery'), $pp_gallery_li = $pp_gallery
						.find('li'); // Set the gallery selectors
			...
			$pp_pic_holder.attr('class', 'pp_pic_holder ' + settings.theme); // Set
																				// the
																				// proper
																				// theme
	...
	function getHashtag() {
		...
		hashtag = (url.indexOf('#prettyPhoto') !== -1)
				? decodeURI(url.substring(url.indexOf('#prettyPhoto') + 1,
						url.length))
				: false;
