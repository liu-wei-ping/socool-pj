...
function getMonthCn(month) {
	var months_cn = [ '一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月',
			'十月', '十一月', '十二月' ];
	for (var $i = 1; $i <= 12; $i++) {
		var m = $i < 10 ? "0" + $i : $i;
		if (month == $i) {
...
function messageBox(flag, successMsg, errorMsg, callback, title, timeOut) {
	...
	if (flag) {
		icon = "success";
		msg = successMsg != undefined && successMsg != "" ? successMsg
				: "操作成功!";
	} else {
		icon = "error";
...
function uploadFile(e) {
	...
	if (file != "") {
		...
		$.messager.confirm("确认操作", "确定要上传[<font color='red'>" + fileName
				+ "</font>]吗？", function(data) {
			if (data) {
				$("#" + formId).submit();
			}
		})
...
$.extend($.fn.validatebox.defaults.rules, {
	// TimeCheck:日期时间比较验证扩展 调用DEMO
	// validType="TimeCheck['apply_start_time','apply_end_time','gt']"
	TimeCheck : {
		validator : function(value, param) {
			...
			var compare = (param[2] || "gt").toLocaleLowerCase();// gt:大于，lt:小于
																	// 默认大于
		...
		.extend({
			messageBox : function(options) {
				...
				if (options.icon != undefined) {
					_msg += '<span class="messager-icon messager-'
							+ options.icon + '"></span>';
				}
				if (options.msg != undefined) {
					_msg += '<span style="word-break:break-all;font-size: 25px;font-weight:500;">'
							+ options.msg + '</span>';
				}
				_msg += '</div>';
				options.msg = _msg;
